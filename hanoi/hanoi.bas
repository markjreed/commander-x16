...
1860 :     T(SP) = FP + PD
1870 :     PL(T(SP)) = 1
1880 :     ?"1880":GOSUB 2800 : REM DRAW CURRENT SETUP
...
3030 SP = SP + 1 : GOSUB 3000 : SP = SP - 1
3040 F = F(SP - 1) : T = T(SP - 1) : GOSUB 3200 : REM MOVE ONE DISK
3050 N(SP) = N(SP - 1) - 1 : F(SP) = V(SP - 1) : T(SP) = T(SP - 1)
3060 SP = SP + 1 : GOSUB 3000 : SP = SP - 1
3070 RETURN
3080 REM
3199 REM MOVE ONE DISK FROM F TO T
3200 H1 = P(F, 0)     : REM STARTING HEIGHT
3210 TD = P(F, H1)     : REM WHICH DISK TO MOVE
...
3280 IF PL(F) <> 1 AND H1 = 1 THEN PL(F) = 0
3290 IF PL(F) = 3 THEN PL(F) = 2
...
3470 IF PL(T) < 2 THEN PL(T) = 2 : REM BUMP PEG LABEL
